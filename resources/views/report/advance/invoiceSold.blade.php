...
<head>
    <title>Advanced Report-Sold</title>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <div class="invoice text-end">
                                        <h1>Invoice</h1>
                                        <p class="mb-1">Invoice Number <span>#{{$filteredSold->first()->first()->receipt}}</span></p>
                                        <p class="mb-0">Invoice Date : <span id="invoice-date">{{ \Carbon\Carbon::parse($filteredSold->first()->first()->date)->format('d-m-Y') }}</span></p>
                                    </div>
                        ...
                        <div class="invoice-info">
                            <div class="row">
                                <div class="col-sm-6 mb-30">
                                    <div class="invoice-number">
                                        ...
                                        <p class="invo-addr-1">
                                            @if($filteredSold->first()->first()->customer==null)
                                            {{ $filteredSold->first()->first()->salesuser->username }} <br>
                                            {{ $filteredSold->first()->first()->salesuser->email }}
                                            <br>{{ $filteredSold->first()->first()->salesuser->phone }} <br>
                        
                                            @else
                                            {{ $filteredSold->first()->first()->customer->customer_name }} <br>
                                            {{ $filteredSold->first()->first()->customer->email }}
                                            <br>{{ $filteredSold->first()->first()->customer->phone }} <br>
                                            {{ $filteredSold->first()->first()->customer->address }} ,
                                            {{ $filteredSold->first()->first()->customer->district }} ,
                                            {{ $filteredSold->first()->first()->customer->city }} <br>  
                                            @endif
                                ...
                                <div class="col-sm-6 mb-30">
                                    <div class="invoice-number text-end">
                                        ...
                                        <p class="invo-addr-1" style="font-weight: 600">
                                            {{ $filteredSold->first()->first()->salesOrder->status }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-sm-6 text-end mb-30">
                                    ...
                                    <p class="inv-from-1">{{ $filteredSold->first()->first()->salesOrder->status === 'Completed' ? 'Paid' : 'Unpaid' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="order-summary">
                            <div class="table-responsive">
                                <table class="table invoice-table">
                                    ...
                                    <tbody>
                                        @foreach ($filteredSold as $index => $filteredSolds)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>
                                                    <div class="item-desc-1">
                                                        <span>{{ $filteredSolds->product->name }}</span>
                                                    </div>
                                                </td>
                                                <td class="text-center">{{ $filteredSolds->quantity }}</td>
                                        ...
                                        <tr>
                                            <td colspan="2" class="text-center fw-bold">Total Quanitity</td>
                                            <td class="text-right fw-bold">{{ $filteredSold->sum(function ($sale) {
