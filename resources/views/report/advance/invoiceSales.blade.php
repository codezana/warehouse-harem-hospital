...
<head>
    <title>Advanced Report-Sales</title>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <div class="invoice text-end">
                                        <h1>Invoice</h1>
                                        <p class="mb-1">Invoice Number <span>#{{$filteredSales->first()->first()->receipt}}</span></p>
                                        <p class="mb-0">Invoice Date : <span id="invoice-date">{{ \Carbon\Carbon::parse($filteredSales->first()->first()->date)->format('d-m-Y') }}</span></p>
                                    </div>
                        ...
                        <div class="invoice-info">
                            <div class="row">
                                <div class="col-sm-6 mb-30">
                                    <div class="invoice-number">
                                        ...
                                        <p class="invo-addr-1">
                                            @if($filteredSales->first()->first()->customer==null)
                                            {{ $filteredSales->first()->first()->salesuser->username }} <br>
                                            {{ $filteredSales->first()->first()->salesuser->email }}
                                            <br>{{ $filteredSales->first()->first()->salesuser->phone }} <br>
                        
                                            @else
                                            {{ $filteredSales->first()->first()->customer->customer_name }} <br>
                                            {{ $filteredSales->first()->first()->customer->email }}
                                            <br>{{ $filteredSales->first()->first()->customer->phone }} <br>
                                            {{ $filteredSales->first()->first()->customer->address }} ,
                                            {{ $filteredSales->first()->first()->customer->district }} ,
                                            {{ $filteredSales->first()->first()->customer->city }} <br>  
                                            @endif
                                ...
                                <div class="col-sm-6 mb-30">
                                    <div class="invoice-number text-end">
                                        ...
                                        <p class="invo-addr-1" style="font-weight: 600">
                                            {{ $filteredSales->first()->first()->salesOrder->status }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-sm-6 text-end mb-30">
                                    ...
                                    <p class="inv-from-1">{{ $filteredSales->first()->first()->salesOrder->status === 'Completed' ? 'Paid' : 'Unpaid' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                                ...
                                <table class="table invoice-table">
                                    ...
                                    <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="item-desc-1">
                                                        <span>{{ $filteredSale->product->name }}</span>
                                                    </div>
                                                </td>
                                                <td class="text-center">{{ number_format($filteredSale->price,0) }}</td>
                                                <td class="text-center">{{ $filteredSale->quantity }}</td>
                                                <td class="text-center">
                                                    {{ number_format($filteredSale->subtotal,0) }}</td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-right fw-bold">{{ $filteredSales->sum(function ($sale) {
                                                return $sale->salesOrder->total_dollar;
                                            }) }}</td>                                        </tr>
                                    </tbody>
                                </table>
                            </div>
