...
@section('content')
    ...
    @php
        $requiredPermissions = ['View Product', 'Select all Product', 'Delete Product', 'Edit Product', 'Create Product'];
    ...
    <div class="page-wrapper page-wrapper-one">
        <div class="content">
            <div class="page-header">
                ...
                <div class="page-btn">
                    @if ($showAddColumn)
                        <a href="{{ route('addbrandpage') }}" class="btn btn-added"><img src="assets/img/icons/plus.svg" class="me-2"
                                alt="img">Add Brand</a>
                    @endif
            ...
            @if (session('error'))
                <div class="alert alert-danger">{{ session('error') }} <i class='bx bx-cool'></i></div>
            @endif
            <div class="card">
                <div class="card-body">
                    <div class="table-top">
                                ...
                                @foreach ($brand as $brands)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if(isset($brands->image))
                                            <a class="product-img">
                                                <img src="{{ asset('uploads/product/brands/' . $brands->image) }}"
                                                    alt="product">
                                            </a>
                                            @else
                                        ...
                                        <td class="description-cell" id="description-cell-{{ $loop->index }}">
                                            <span
                                                class="truncated-description">{{ Str::limit($brands->description, 25) }}</span>
                                            @if (strlen($brands->description) > 25)
                                                <a href="#" class="show-description-modal"
                                                    data-modal-id="modal-{{ $loop->index }}">Read More</a>
                                            @endif
                                        </td>
                                        <td>{{ $brands->createdByUser ? $brands->createdByUser->username : 'N/A' }}</td>
                                        ...
                                        @if ($showActionColumn)
                                            <td>
                                                @if ($showEditColumn)
                                                    <a class="me-3"
                                                        href="{{ route('product.brand.edit', ['id' => $brands->id]) }}">
                                                        <img src="assets/img/icons/edit.svg" alt="Edit">
                                                    </a>
                                                @endif
                                                @if ($showDeleteColumn)
                                                <form action="{{ route('product.brand.delete', ['id' => $brands->id]) }}"
                                                    style="display: inline" method="POST" class="delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="me-3 delete-button" href="javascript:void(0);"
                                                        data-user-id="{{ $brands->id }}">
                                                        <img src="assets/img/icons/delete.svg" alt="img">
                                                    </a>

                                                    <div id="deleteModal{{ $brands->id }}" class="modal">
                                    ...
                                    <div id="modal-{{ $loop->index }}" class="modal">
                                        <div class="modal-content">
                                            ...
                                            <p class="full-description">{{ $brands->description }}</p>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                <script>
                                    $(document).ready(function() {
                                        $('.delete-button').click(function() {
                                            var brandId = $(this).data('user-id'); // Retrieve product ID from data-user-id attribute
                                            $('#deleteModal' + brandId).show();
                                        });
...
@section('custom-js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#brandName,#CreatedBy").on("keyup change",
                function() {
                    const brandName = $("#brandName").val().toLowerCase();
            ...
            clearButton.on("click", function() {
                $("#brandName,#CreatedBy").val(""); // Clear input values
    ...
    <script>
        ...
        deleteButtons.forEach(deleteButton => {
            ...
            const closeButton = document.querySelector(`#deleteModal${userId} .close`);
            closeButton.addEventListener('click', () => {
                closeModal(userId);
            });

            const confirmButton = document.querySelector(`#deleteModal${userId} .confirmDelete`);
            confirmButton.addEventListener('click', () => {
                // Perform the delete action here for the user with the specified userId
